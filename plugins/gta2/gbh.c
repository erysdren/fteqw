...
static struct gbhblockinfo city_scape[8][256][256];

static struct gbhchunk *GBH_FindChunk(const char *type, uint8_t *buffer, size_t fsize)
{
	uint8_t *ptr = buffer;
	...
	while (ptr < end)
	{
		struct gbhchunk *chunk = (struct gbhchunk *)ptr;
		if (memcmp(&chunk->magic, type, 4) == 0)
			return chunk;
		ptr += sizeof(struct gbhchunk) + chunk->size;
	}

	return NULL;
}

static qboolean GBH_LoadCMAP(model_t *mod, struct gbhchunk *chunk)
{
	uint16_t *base;
	uint16_t column_words;
	uint16_t *column;
	uint16_t num_blocks;
	struct gbhblockinfo *blocks;
	int x, y;
	...
	blocks = (struct gbhblockinfo *)(column + column_words + 1);
	...
	for (y = 0; y < 256; y++)
	{
		for (x = 0; x < 256; x++)
		{
			...
			for (i = 0; i < height; i++)
			{
				...
				if (i >= offset)
				{
					memcpy(&city_scape[i][y][x], &blocks[blockd[i - offset]], sizeof(struct gbhblockinfo));
				}
			}
		}
	}
...
static qboolean GBH_LoadDMAP(model_t *mod, struct gbhchunk *chunk)
{
	uint32_t *base;
...
static qboolean GBH_LoadUMAP(model_t *mod, struct gbhchunk *chunk)
{
	// sanity check
	if (chunk->size < sizeof(city_scape))
	{
		Con_Printf("%s: UMAP chunk size mismatch\n", mod->name);
		return false;
	}

	memcpy(city_scape, (void *)(chunk + 1), sizeof(city_scape));
...
static qboolean QDECL Mod_LoadGBH(model_t *mod, void *buffer, size_t fsize)
{
	...
	struct gbhheader *header = NULL;
	struct gbhchunk *chunk = NULL;
	...
	header = (struct gbhheader *)buffer;
	...
	// clear city scape
	memset(city_scape, 0, sizeof(city_scape));
	...
	for (z = 0; z < 8; z++)
	{
		for (y = 0; y < 256; y++)
		{
			for (x = 0; x < 256; x++)
			{
				if (GROUND_TYPE(city_scape[z][y][x].slope_type) == GROUND_TYPE_ROAD)
				{
					gai[0].numverts += 4;
					gai[0].numindexes += 6;
				}
		...
		for (y = 0; y < 256; y++)
		{
			for (x = 0; x < 256; x++)
			{
				if (GROUND_TYPE(city_scape[z][y][x].slope_type) == GROUND_TYPE_ROAD)
				{
					...
					// setup positions
					gai[0].ofs_skel_xyz[dstverts[0]][0] = (x * 64);
					gai[0].ofs_skel_xyz[dstverts[0]][1] = (y * 64);
					gai[0].ofs_skel_xyz[dstverts[0]][2] = (z * 64);

					gai[0].ofs_skel_xyz[dstverts[1]][0] = (x * 64);
					gai[0].ofs_skel_xyz[dstverts[1]][1] = (y * 64) + 64;
					gai[0].ofs_skel_xyz[dstverts[1]][2] = (z * 64);

					gai[0].ofs_skel_xyz[dstverts[2]][0] = (x * 64) + 64;
					gai[0].ofs_skel_xyz[dstverts[2]][1] = (y * 64) + 64;
					gai[0].ofs_skel_xyz[dstverts[2]][2] = (z * 64);

					gai[0].ofs_skel_xyz[dstverts[3]][0] = (x * 64) + 64;
					gai[0].ofs_skel_xyz[dstverts[3]][1] = (y * 64);
