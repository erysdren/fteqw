...
static void World_Jolt_BodyFromEntity(world_t *world, wedict_t *ed)
{
	...
	if (modified && body)
	{
		...
		if (!VectorCompare(angles, ed->rbe.angles))
		{
			...
			body_interface.SetRotation(body->GetID(), JPH::Quat::sEulerAngles(JPH::Vec3Arg(DEG2RAD(angles[PITCH]), DEG2RAD(angles[ROLL]), DEG2RAD(angles[YAW]))), JPH::EActivation::Activate);
		}
