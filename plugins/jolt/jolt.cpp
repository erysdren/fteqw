...
static void World_Jolt_RunCommand(world_t *world, rbecommandqueue_t *cmd)
{
	...
	switch (cmd->command)
	{
		...
		case RBECMD_FORCE:
		{
			...
			body_interface.AddImpulse(body_id, JPH::Vec3Arg(cmd->v1[0], cmd->v1[1], cmd->v1[2]), JPH::RVec3(cmd->v2[0], cmd->v2[1], cmd->v2[2]) * InchesToMeters);
			break;
		}
...
static void World_Jolt_BodyFromEntity(world_t *world, wedict_t *ed)
{
	...
	if (!VectorCompare(angles, ed->rbe.angles) || !VectorCompare(origin, ed->rbe.origin) || !VectorCompare(velocity, ed->rbe.velocity) || !VectorCompare(avelocity, ed->rbe.avelocity) || gravity != ed->rbe.gravity)
		modified = qtrue;
	...
	if (modified && body)
	{
		...
		if (!VectorCompare(angles, ed->rbe.angles))
		{
			...
			body_interface.SetRotation(body->GetID(), JPH::Quat::sEulerAngles(JPH::Vec3Arg(DEG2RAD(angles[YAW]), DEG2RAD(angles[ROLL]), DEG2RAD(angles[PITCH]))), JPH::EActivation::Activate);
		}
...
#if 0 // TODO figure out why this is crash happy
		if (ed->xv->gravity != body_interface.GetGravityFactor(body->GetID()))
		{
			body_interface.SetGravityFactor(body->GetID(), ed->xv->gravity);
		}

		if (ed->xv->friction != body_interface.GetFriction(body->GetID()))
		{
...
static void World_Jolt_BodyToEntity(world_t *world, wedict_t *ed)
{
	...
	VectorCopy(angles, ed->rbe.angles);
	VectorCopy(avelocity, ed->rbe.avelocity);
	ed->rbe.gravity = body_interface.GetGravityFactor(body->GetID()) ? qtrue : qfalse;
...
static void World_Jolt_JointFromEntity(world_t *world, wedict_t *ed)
{
	...
	int joint_type = ed->xv->jointtype;
	...
	if (joint_type == ed->rbe.joint_type &&
...
static bool Plug_Jolt_AssertFailedImpl(const char *inExpression, const char *inMessage, const char *inFile, uint inLine)
{
	char buffer[1024];
